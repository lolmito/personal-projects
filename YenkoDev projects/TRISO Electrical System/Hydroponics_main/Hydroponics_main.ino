...
*/
  ...
  pinMode(RELAY_HYDRO, OUTPUT);
  pinMode(RELAY_GRID, OUTPUT);
  digitalWrite(RELAY_SOLAR, LOW);
  digitalWrite(RELAY_HYDRO, LOW);
...
// Membership function for voltage classification
float fuzzifyLow(float voltage) {
    return voltage <= 11.5 ? 1.0 : (voltage <= 12.6 ? (12.6 - voltage) / 1.1 : 0.0);
}

float fuzzifyMedium(float voltage) {
    return (voltage > 11.5 && voltage <= 12.6) ? ((voltage - 11.5) / 1.1) : ((voltage > 12.6 && voltage <= 13.7) ? (13.7 - voltage) / 1.1 : 0.0);
}

float fuzzifyHigh(float voltage) {
    return voltage >= 13.7 ? 1.0 : (voltage >= 12.6 ? (voltage - 12.6) / 1.1 : 0.0);
}

void selectPowerSource(float solarVoltage, float hydroVoltage) {
    float solarLow = fuzzifyLow(solarVoltage);
    float solarMedium = fuzzifyMedium(solarVoltage);
    float solarHigh = fuzzifyHigh(solarVoltage);

    float hydroLow = fuzzifyLow(hydroVoltage);
    float hydroMedium = fuzzifyMedium(hydroVoltage);
    float hydroHigh = fuzzifyHigh(hydroVoltage);
    ...
    } else if (hydroHigh > 0.5) {
        digitalWrite(RELAY_SOLAR, LOW);
        digitalWrite(RELAY_HYDRO, HIGH);
        digitalWrite(RELAY_GRID, LOW);
        Serial.println("Using Hydro Power");
    } else if (solarMedium > 0.5 && hydroMedium > 0.5) {
        digitalWrite(RELAY_SOLAR, HIGH);
        digitalWrite(RELAY_HYDRO, HIGH);
        digitalWrite(RELAY_GRID, LOW);
        Serial.println("Using Solar + Hydro Power");
    } else {
        digitalWrite(RELAY_SOLAR, LOW);
        digitalWrite(RELAY_HYDRO, LOW);
